...
if not os.path.isfile(checkpoint_name):
    urlretrieve('http://mirror.io.community/blob/vqgan/' + checkpoint_name, checkpoint_name)
if not os.path.isfile(config_name):
    urlretrieve('http://mirror.io.community/blob/vqgan/' + config_name, config_name)
...
args = argparse.Namespace(
    ...
    tv_weight=0.,
    clip_model='ViT-B/32',
    vqgan_config=config_name,
    vqgan_checkpoint=checkpoint_name,
    step_size=0.05,
    weight_decay=0.,
    cutn=64,
    cut_pow=1.,
...
def checkin(i, losses):
    ...
    out = synth(one_hot)
    TF.to_pil_image(out[0].cpu()).save('progress_{}.png'.format(i))
